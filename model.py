        ...
        def SegmentNet(input, scope, is_training, reuse=None):
            with tf.variable_scope(scope, reuse=reuse):
                with slim.arg_scope([slim.conv2d],
                    ...
                    net = slim.conv2d(input, 16, [3, 3], scope='conv1')     # conv1
                    net = slim.conv2d(net, 16, [3, 3], scope='conv2')       # conv2
                    net = slim.max_pool2d(net, [2, 2], [2, 2], scope='pool1')   # pool1

                    net = slim.conv2d(net, 32, [3, 3], scope='conv3')       # conv3
                    net = slim.conv2d(net, 32, [3, 3], scope='conv4')       # conv4
                    #net = slim.conv2d(net, 32, [3, 3], scope='conv5')
                    net = slim.max_pool2d(net, [2, 2], [2, 2], scope='pool2')   # pool2

                    net = slim.conv2d(net, 32, [3, 3], scope='conv6')       # conv5
                    #net = slim.conv2d(net, 32, [3, 3], scope='conv7')
                    net = slim.conv2d(net, 32, [3, 3], scope='conv8')       # conv6
                    net = slim.conv2d(net, 32, [3, 3], scope='conv9')       # conv7
                    net = slim.max_pool2d(net, [2, 2], [2, 2], scope='pool3')      # pool3
                    net1 = slim.conv2d(net, 128, [1, 1], scope='conv10')    # conv8
                    net2 = tf.keras.layers.Conv2D(
                        128, 3, strides=1, padding="same", dilation_rate=3)(net)    # conv9
                    m1 = conv_concat(net1, net2, 1)
                    net2 = tf.keras.layers.Conv2D(
                        128, 3, strides=1, padding="same", dilation_rate=6)(net)    # conv10
                    m2 = conv_concat(m1, net2, 2)
                    net2 = tf.keras.layers.Conv2D(
                        128, 3, strides=1, padding="same", dilation_rate=12)(net)   # conv11
                    net = conv_concat(m2, net2, 3)
                    net = slim.conv2d(net, 512, [1, 1], scope='conv11')             # conv12
                    features = net
                    net = slim.conv2d(net, 1, [1, 1], activation_fn=None, scope='conv17')   # conv13
                    logits_pixel = net
